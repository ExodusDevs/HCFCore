...
class RegionManager
{
  ...
  public function saveRegion(string $name): void
  {
    ...
    Filesystem::safeFilePutContents($this->loader->getDataFolder() . "regions" . DIRECTORY_SEPARATOR . $name . ".json", json_encode(($this->getRegion($name))->jsonSerialize(), JSON_PRETTY_PRINT | JSON_BIGINT_AS_STRING | JSON_THROW_ON_ERROR));
  }
  ...
  public function getRegionInLocation(Position $position): ?Region
  {
    $regionLocation = null;
    foreach($this->regions as $region) {
    ...
    return $regionInLocation;
  }
  
  public function getRegionsInLocation(Position $position): array
  {
    $regions = [];
    foreach($this->regions as $region) {
      if (!$region->isInside($position)) continue;
        $regions[] = $region;
      }
    }
    return $regions;
